...
# Build and bundle CoderClaw into a minimal .app we can open.
# Outputs to dist/CoderClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/CoderClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="CoderClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.coderclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/coderclaw/coderclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/CoderClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :CoderClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :CoderClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/CoderClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/CoderClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/CoderClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/CoderClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/CoderClaw/Resources/CoderClaw.icns" "$APP_ROOT/Contents/Resources/CoderClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/CoderClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying CoderClawKit resources"
CODERCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/CoderClawKit_CoderClawKit.bundle"
if [ -d "$CODERCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/CoderClawKit_CoderClawKit.bundle"
  cp -R "$CODERCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/CoderClawKit_CoderClawKit.bundle"
else
  echo "WARN: CoderClawKit resource bundle not found at $CODERCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running CoderClaw"
killall -q CoderClaw 2>/dev/null || true
