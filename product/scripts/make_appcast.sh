...
ZIP=${1:?"Usage: $0 CoderClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/coderclaw/coderclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^CoderClaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/seanhogg/coderclaw/releases/download/v${VERSION}/"}
