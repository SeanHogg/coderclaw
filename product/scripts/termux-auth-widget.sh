...
# CoderClaw Auth Widget for Termux
...
SERVER="${CODERCLAW_SERVER:-${CODERCLAW_SERVER:-l36}}"

# Check auth status
termux-toast "Checking CoderClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/coderclaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/coderclaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|CODERCLAW_EXPIRING|CODERCLAW_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/coderclaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|CODERCLAW_EXPIRED|CODERCLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: CoderClaw won't work until re-auth"
                ;;
        esac
        ;;
