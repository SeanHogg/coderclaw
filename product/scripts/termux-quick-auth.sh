...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/coderclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "CoderClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/coderclaw/scripts/mobile-reauth.sh'" --id coderclaw-auth
        ;;
    *)
