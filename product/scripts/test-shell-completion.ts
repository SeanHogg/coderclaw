 ...
 * `coderclaw update`. Use it to verify the completion installation flow
...
\`coderclaw update\`, \`coderclaw doctor\`, and \`coderclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# CoderClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`coderclaw completion --install\` later to enable.`));
    return;
  }
