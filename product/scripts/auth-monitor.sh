...
#
# Suggested cron: */30 * * * * /home/admin/coderclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send CoderClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., coderclaw-alerts)
...
STATE_FILE="$HOME/.coderclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via CoderClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use coderclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via CoderClaw to $NOTIFY_PHONE..."
            coderclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! CoderClaw is down. Run: ssh l36 '~/coderclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
