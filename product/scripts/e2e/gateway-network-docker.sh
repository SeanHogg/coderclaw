...
IMAGE_NAME="coderclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="coderclaw-net-e2e-$$"
GW_NAME="coderclaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "CODERCLAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "CODERCLAW_SKIP_CHANNELS=1" \
	  -e "CODERCLAW_SKIP_GMAIL_WATCHER=1" \
	  -e "CODERCLAW_SKIP_CRON=1" \
	  -e "CODERCLAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
