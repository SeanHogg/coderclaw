#!/usr/bin/env bash
# ClawDock - Docker helpers for CoderClaw
# Inspired by Simon Willison's "Running CoderClaw in Docker"
# https://til.simonwillison.net/llms/coderclaw-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/coderclaw/coderclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for CoderClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/coderclaw"
  "${HOME}/workspace/coderclaw"
  "${HOME}/projects/coderclaw"
  "${HOME}/dev/coderclaw"
  "${HOME}/code/coderclaw"
  "${HOME}/src/coderclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found CoderClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/coderclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ CoderClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/seanhogg/coderclaw.git ~/coderclaw"
    echo "  cd ~/coderclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^CODERCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d coderclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart coderclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f coderclaw-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec coderclaw-gateway \
    bash -c 'echo "alias coderclaw=\"./coderclaw.mjs\"" > /tmp/.bashrc_coderclaw && bash --rcfile /tmp/.bashrc_coderclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm coderclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build coderclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "CODERCLAW_GATEWAY_TOKEN=$token" coderclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" coderclaw-gateway \
    bash -c './coderclaw.mjs config set gateway.remote.token "$TOKEN" && ./coderclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec coderclaw-gateway \
    bash -c "./coderclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart coderclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm coderclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec coderclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'coderclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for CoderClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (coderclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to coderclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.coderclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'coderclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'coderclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.coderclaw.ai${_CLR_RESET}"
  echo ""
}
