...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("coderclaw-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("coderclaw-bin-sherpa-");
      const modelDir = await makeTempDir("coderclaw-sherpa-model-");
      ...
      const cfg: CoderClawConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("coderclaw-bin-whispercpp-");
      const modelDir = await makeTempDir("coderclaw-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("coderclaw-bin-gemini-");
      ...
      const cfg: CoderClawConfig = { tools: { media: { image: {} } } };
