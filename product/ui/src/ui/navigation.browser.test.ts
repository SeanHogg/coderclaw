...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/coderclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/coderclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/coderclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__CODERCLAW_CONTROL_UI_BASE_PATH__ = "/coderclaw";
    const app = mountApp("/coderclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/coderclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/coderclaw/sessions");
  });
