...
CoderClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
coderclaw channels login --channel zalouser              # Login via QR
coderclaw channels login --channel zalouser --account work
...
```bash
# Text
coderclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
coderclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `coderclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
coderclaw channels login --channel zalouser --account work
coderclaw message send --channel zalouser --account work --target <id> --message "Hello"
