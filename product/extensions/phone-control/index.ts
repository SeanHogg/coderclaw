...
function normalizeDenyList(cfg: CoderClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: CoderClawPluginApi["config"]): string[] {
...
export default function register(api: CoderClawPluginApi) {
  ...
  const timerService: CoderClawPluginService = {
