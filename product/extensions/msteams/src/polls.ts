...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["coderclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["coderclaw", "pollId"]) ??
    readNestedString(value, ["coderclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "coderclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "coderclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          coderclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "coderclaw poll vote",
          displayText: "Vote recorded",
          value: { coderclawPollId: pollId, pollId },
        },
      },
    ],
  };
