...
function resolveGatewayPort(cfg: CoderClawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.CODERCLAW_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.CODERCLAW_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
async function resolveTailnetHost(api: CoderClawPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: CoderClawPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: CoderClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the CoderClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the CoderClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
