...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `coderclaw-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "coderclaw.plugin.json"), JSON.stringify(manifest), "utf-8");
}
