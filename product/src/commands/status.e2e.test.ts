...
beforeAll(() => {
  envSnapshot = captureEnv(["CODERCLAW_PROFILE"]);
  process.env.CODERCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/coderclaw-root.js", () => ({
  resolveCoderClawPackageRoot: vi.fn().mockResolvedValue("/tmp/coderclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/coderclaw/pnpm-lock.yaml",
      markerPath: "/tmp/coderclaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("CoderClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("coderclaw status --all") ||
          l.includes("coderclaw --profile isolated status --all") ||
          l.includes("coderclaw status --all") ||
          l.includes("coderclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.CODERCLAW_GATEWAY_TOKEN = "abcd1234";
    try {
