...
export async function loadValidConfigOrThrow(): Promise<CoderClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: CoderClawConfig) => CoderClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: CoderClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: CoderClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("coderclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
