...
const SEARCH_TOOL = "https://docs.coderclaw.ai/mcp.SearchCoderClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.coderclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('coderclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.coderclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('coderclaw docs "your query"')}`);
    }
    return;
  }
