...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "CODERCLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("CODERCLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv CODERCLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("CODERCLAW_GATEWAY_PASSWORD");
  });
