...
function resolveAgentName(cfg: CoderClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: CoderClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: CoderClawConfig): AgentSummary[] {
