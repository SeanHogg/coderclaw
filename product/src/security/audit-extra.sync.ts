...
function summarizeGroupPolicy(cfg: CoderClawConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: CoderClawConfig): boolean {
...
function collectModels(cfg: CoderClawConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: CoderClawConfig): Set<string> {
  const baseCfg: CoderClawConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: CoderClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: CoderClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: CoderClawConfig): boolean {
...
function isBrowserEnabled(cfg: CoderClawConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: CoderClawConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: CoderClawConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep CODERCLAW_STATE_DIR on a local-only volume and re-run "${formatCliCommand("coderclaw security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: CoderClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer CODERCLAW_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const coderclawGatewayToken =
    typeof env.CODERCLAW_GATEWAY_TOKEN === "string" && env.CODERCLAW_GATEWAY_TOKEN.trim()
      ? env.CODERCLAW_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-coderclaw-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: CoderClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: CoderClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: CoderClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
