...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: CoderClawConfig) => unknown;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: CoderClawConfig = { session: { dmScope: "main" } };
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: CoderClawConfig = { gateway: { mode: "local" } };
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: CoderClawConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        coderclaw: { extensions: ["../outside.js"] },
      }),
    );
