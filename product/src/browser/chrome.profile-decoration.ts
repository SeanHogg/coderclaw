...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".coderclaw-profile-decorated");
}
 ...
 */
export function decorateCoderClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_CODERCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_CODERCLAW_BROWSER_COLOR).toUpperCase();
