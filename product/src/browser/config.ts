import type { BrowserConfig, BrowserProfileConfig, CoderClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_CODERCLAW_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "coderclaw" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_CODERCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "coderclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_CODERCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_CODERCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an CoderClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for coderclaw/coderclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: CoderClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_CODERCLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "coderclaw";
