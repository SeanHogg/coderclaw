...
function resolveGatewayPort(cfg: CoderClawConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: CoderClawConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.CODERCLAW_GATEWAY_PASSWORD?.trim() ||
    env.CODERCLAW_GATEWAY_PASSWORD?.trim() ||
