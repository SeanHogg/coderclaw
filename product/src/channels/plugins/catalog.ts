...
import { discoverCoderClawPlugins } from "../../plugins/discovery.js";
import type { CoderClawPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, CoderClawPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["CODERCLAW_PLUGIN_CATALOG_PATHS", "CODERCLAW_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<CoderClawPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: CoderClawPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: CoderClawPackageManifest;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  ...
  const manifest =
    ...
    LEGACY_MANIFEST_KEYS.map(
      (k) => (entry as Record<string, unknown>)[k] as (typeof entry)[typeof MANIFEST_KEY],
    ).find(Boolean);
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverCoderClawPlugins({ workspaceDir: options.workspaceDir });
