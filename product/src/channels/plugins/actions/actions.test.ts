...
function telegramCfg(): CoderClawConfig {
  return { channels: { telegram: { botToken: "tok" } } } as CoderClawConfig;
}

function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as CoderClawConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as CoderClawConfig;
