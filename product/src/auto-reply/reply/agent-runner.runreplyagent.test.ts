...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { CODERCLAW_STATE_DIR: process.env.CODERCLAW_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.CODERCLAW_STATE_DIR === undefined) {
      delete process.env.CODERCLAW_STATE_DIR;
    } else {
      process.env.CODERCLAW_STATE_DIR = snapshot.CODERCLAW_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "coderclaw-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "coderclaw-memory-flush-"));
  });
