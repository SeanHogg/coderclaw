...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: CoderClawConfig | null = null;
  ...
  get(key: string, cfg: CoderClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: CoderClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: CoderClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: CoderClawConfig): void {
