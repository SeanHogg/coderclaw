...
const DEFAULT_PACKAGE_NAME = "coderclaw";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "coderclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("coderclaw doctor", doctorArgv, gitRoot, { CODERCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
