...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.CODERCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*coderclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "coderclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "coderclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "coderclaw-gw");
  ...
  records.push(`_coderclaw-gw._tcp IN PTR ${instanceLabel}._coderclaw-gw._tcp`);
  records.push(`${instanceLabel}._coderclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._coderclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; coderclaw-content-hash: ${contentHash}\n${contentBody}`;
}
