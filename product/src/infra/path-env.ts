...
function candidateBinDirs(opts: EnsureCoderClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `coderclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "coderclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.CODERCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "coderclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `coderclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureCoderClawCliOnPath(opts: EnsureCoderClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.CODERCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
