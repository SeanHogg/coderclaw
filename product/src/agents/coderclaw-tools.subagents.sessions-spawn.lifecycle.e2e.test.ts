...
} from "./coderclaw-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateCoderClawTools = (typeof import("./coderclaw-tools.js"))["createCoderClawTools"];
type CreateCoderClawToolsOpts = Parameters<CreateCoderClawTools>[0];

async function getSessionsSpawnTool(opts: CreateCoderClawToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createCoderClawTools } = await import("./coderclaw-tools.js");
  const tool = createCoderClawTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("coderclaw-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
