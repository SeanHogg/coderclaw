...
import { createCoderClawTools } from "./coderclaw-tools.js";
import { __testing, createCoderClawCodingTools } from "./pi-tools.js";
import { createCoderClawReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createCoderClawCodingTools();
...
describe("createCoderClawCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createCoderClawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createCoderClawCodingTools({ messageProvider: "discord" });
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createCoderClawCodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "coderclaw-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createCoderClawReadTool(
      baseRead as unknown as Parameters<typeof createCoderClawReadTool>[0],
    );
