...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.coderclaw.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", CODERCLAW_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    ...
    const label = "ai.coderclaw.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when CODERCLAW_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.coderclaw.gateway.plist",
    );
  });

  it("uses profile-specific label when CODERCLAW_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", CODERCLAW_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.coderclaw.jbphoenix.plist",
    );
  });

  it("prefers CODERCLAW_LAUNCHD_LABEL over CODERCLAW_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      CODERCLAW_PROFILE: "jbphoenix",
      CODERCLAW_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from CODERCLAW_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      CODERCLAW_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty CODERCLAW_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      CODERCLAW_PROFILE: "myprofile",
      CODERCLAW_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.coderclaw.myprofile.plist",
    );
  });
});
