...
export function hasHelpOrVersion(argv: string[]): boolean {
  const hasFlag = argv.some((arg) => HELP_FLAGS.has(arg) || VERSION_FLAGS.has(arg));
  if (hasFlag) {
    return true;
  }
  return getPrimaryCommand(argv) === "help";
}
...
export function buildParseArgv(params: {
  ...
  const normalizedArgv =
    programName && baseArgv[0] === programName
      ? baseArgv.slice(1)
      : baseArgv[0]?.endsWith("coderclaw") || baseArgv[0]?.endsWith("coderclaw")
        ? baseArgv.slice(1)
        : baseArgv;
  ...
  return ["node", programName || "coderclaw", ...normalizedArgv];
}
