...
export function applyCliProfileEnv(params: {
  ...
  env.CODERCLAW_PROFILE = profile;

  const stateDir = env.CODERCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.CODERCLAW_STATE_DIR?.trim()) {
    env.CODERCLAW_STATE_DIR = stateDir;
  }

  if (!env.CODERCLAW_CONFIG_PATH?.trim()) {
    env.CODERCLAW_CONFIG_PATH = path.join(stateDir, "coderclaw.json");
  }

  if (profile === "dev" && !env.CODERCLAW_GATEWAY_PORT?.trim()) {
    env.CODERCLAW_GATEWAY_PORT = "19001";
  }
}
