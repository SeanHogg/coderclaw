...
describe("shared/frontmatter", () => {
  ...
  test("resolveCoderClawManifestBlock parses JSON5 metadata and picks coderclaw block", () => {
    const frontmatter = {
      metadata: "{ coderclaw: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveCoderClawManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveCoderClawManifestBlock returns undefined for invalid input", () => {
    expect(resolveCoderClawManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveCoderClawManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveCoderClawManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
