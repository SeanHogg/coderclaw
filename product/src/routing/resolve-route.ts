...
function listAgents(cfg: CoderClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: CoderClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: CoderClawConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<CoderClawConfig, EvaluatedBindingsCache>();
