...
import type { CoderClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareCoderClawVersions } from "./version.js";
...
function coerceConfig(value: unknown): CoderClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as CoderClawConfig;
}
...
function stampConfigVersion(cfg: CoderClawConfig): CoderClawConfig {
...
function warnIfConfigFromFuture(cfg: CoderClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareCoderClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer CoderClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as CoderClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): CoderClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as CoderClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: CoderClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as CoderClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.CODERCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.CODERCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.CODERCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.CODERCLAW_WATCH_SESSION === "string" &&
        deps.env.CODERCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.CODERCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.CODERCLAW_WATCH_COMMAND === "string" &&
        deps.env.CODERCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.CODERCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `CODERCLAW_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CODERCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.CODERCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): CoderClawConfig {
