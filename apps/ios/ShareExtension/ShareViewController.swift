...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.coderclaw.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to CoderClaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to CoderClaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to CoderClaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "CoderClawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "CoderClaw is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "CoderClaw Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("coderclaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
