...
final class RemindersService: RemindersServicing {
    func list(params: CoderClawRemindersListParams) async throws -> CoderClawRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[CoderClawReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return CoderClawReminderPayload(
        ...
        return CoderClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: CoderClawRemindersAddParams) async throws -> CoderClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = CoderClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return CoderClawRemindersAddPayload(reminder: payload)
    }
