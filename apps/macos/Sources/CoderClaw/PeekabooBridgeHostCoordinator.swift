...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.coderclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: CoderClawPeekabooBridgeServices?
    private static var coderclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("CoderClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = CoderClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.coderclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.coderclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class CoderClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.coderclaw.peekaboo")
