...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.coderclaw/coderclaw.json. Prefer CODERCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.coderclaw/coderclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.coderclaw/coderclaw.json. Restart the gateway to apply."
        }
