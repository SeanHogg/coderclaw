...
export async function handleWhatsAppStart(host: CoderClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: CoderClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: CoderClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: CoderClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: CoderClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: CoderClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: CoderClawApp): string | null {
...
function buildGatewayHttpHeaders(host: CoderClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: CoderClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: CoderClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: CoderClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: CoderClawApp) {
  const state = host.nostrProfileFormState;
