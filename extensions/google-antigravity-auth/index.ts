...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>CoderClaw Antigravity OAuth</title>
  </head>
  <body>
    <main>
...
function buildAuthUrl(params: { challenge: string; state: string }): string {
  if (!CLIENT_ID) {
    throw new Error("Missing CODERCLAW_GOOGLE_OAUTH_CLIENT_ID");
  }
...
async function exchangeCode(params: {
  ...
  if (!CLIENT_ID || !CLIENT_SECRET) {
    throw new Error(
      "Missing Google OAuth credentials. Set CODERCLAW_GOOGLE_OAUTH_CLIENT_ID and CODERCLAW_GOOGLE_OAUTH_CLIENT_SECRET.",
    );
  }
