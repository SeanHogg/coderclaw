...
import type { CoderClawPluginApi, CoderClawPluginToolContext } from "../../../src/plugins/types.js";
...
function fakeApi(overrides: Partial<CoderClawPluginApi> = {}): CoderClawPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<CoderClawPluginToolContext> = {}): CoderClawPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-lobster-plugin-"));
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: CoderClawPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
