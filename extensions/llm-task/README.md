...
`coderclaw.invoke --each`) without adding new CoderClaw code per workflow.
...
This extension depends on CoderClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** CoderClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.coderclaw/extensions` as a standalone plugin directory.
