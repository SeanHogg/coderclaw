...
metadata: { "coderclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
## CoderClaw Runtime Mapping

- **Task tool** in the upstream spec == CoderClaw `sessions_spawn`
- **File I/O** == CoderClaw `read`/`write`
- **Remote fetch** == CoderClaw `web_fetch` (or `exec` with curl when POST is required)
