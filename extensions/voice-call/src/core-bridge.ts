...
function resolveCoderClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.CODERCLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set CODERCLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveCoderClawRoot(), "dist", "extensionAPI.js");
