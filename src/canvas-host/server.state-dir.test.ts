...
describe("canvas host state dir defaults", () => {
  ...
  it("uses CODERCLAW_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("CoderClaw Canvas");
    } finally {
      await handler.close();
