...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["CODERCLAW_BASE_ENV"]);
    process.env.CODERCLAW_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.CODERCLAW_BASE_ENV ?? "") + "|" + (process.env.CODERCLAW_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { CODERCLAW_TEST_ENV: "ok" },
        },
      );
