...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "CoderClaw is a hobby project and still in beta. Expect sharp edges.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "If you’re not comfortable with basic security and access control, don’t run CoderClaw.",
      ...
      "",
      "Must read: https://docs.coderclaw.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
function hasConfiguredChannels(config: CoderClawConfig): boolean {
  const channels = config.channels;
  if (!channels || typeof channels !== "object") {
    return false;
  }
  return Object.values(channels as Record<string, unknown>).some((value) => {
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("CoderClaw onboarding");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: CoderClawConfig = snapshot.valid ? snapshot.config : {};
  let configHandlingAction: "keep" | "modify" | "reset" | null = null;
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.coderclaw.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("coderclaw doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("coderclaw configure")}.`;
  ...
  const keepExistingValues = configHandlingAction === "keep";
  ...
  const localProbe = await onboardHelpers.probeGatewayReachable({
    url: localUrl,
    token: baseConfig.gateway?.auth?.token ?? process.env.CODERCLAW_GATEWAY_TOKEN,
    password: baseConfig.gateway?.auth?.password ?? process.env.CODERCLAW_GATEWAY_PASSWORD,
  });
  ...
  let nextConfig: CoderClawConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
  ...
  const shouldReuseExistingAuth = keepExistingValues && hasConfiguredModelOrAuth(nextConfig);
  ...
  if (keepExistingValues && quickstartGateway.hasExisting) {
    ...
    await prompter.note("Keeping existing gateway settings.", "Gateway");
  } else {
  ...
  const skipChannelsForKeep = keepExistingValues && hasConfiguredChannels(nextConfig);
  if (opts.skipChannels ?? opts.skipProviders ?? skipChannelsForKeep) {
