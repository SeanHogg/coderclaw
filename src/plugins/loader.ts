...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: CoderClawPluginDefinition;
  register?: CoderClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as CoderClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as CoderClawPluginDefinition;
...
export function loadCoderClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverCoderClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: CoderClawPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as CoderClawPluginModule;
    } catch (err) {
