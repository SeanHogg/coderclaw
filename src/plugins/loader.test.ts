...
import { loadCoderClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `coderclaw-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.CODERCLAW_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "coderclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.CODERCLAW_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadCoderClawPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.CODERCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadCoderClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadCoderClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadCoderClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
