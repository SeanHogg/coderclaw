...
export function isCommandEnabled(cfg: CoderClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: CoderClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as CoderClawConfig),
...
export function getCommandDetection(_cfg?: CoderClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: CoderClawConfig) {
