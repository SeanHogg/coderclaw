...
function createReplyConfig(home: string, streamMode?: "block"): CoderClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("coderclaw-stream-", async (home) => {
    await fs.mkdir(path.join(home, ".coderclaw", "agents", "main", "sessions"), {
      recursive: true,
    });
    return fn(home);
  });
