...
const GOOGLE_LIVE_SINGLE_KEY = "CODERCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "CODERCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "CODERCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "CODERCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `CODERCLAW_LIVE_${base}_KEY`;
