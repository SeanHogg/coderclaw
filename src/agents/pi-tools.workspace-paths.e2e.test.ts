...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("coderclaw-ws-", async (workspaceDir) => {
      await withTempDir("coderclaw-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createCoderClawCodingTools({ workspaceDir });
    ...
    await withTempDir("coderclaw-ws-", async (workspaceDir) => {
      await withTempDir("coderclaw-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello coderclaw");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("coderclaw-ws-", async (workspaceDir) => {
      await withTempDir("coderclaw-override-", async (overrideDir) => {
        const tools = createCoderClawCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("coderclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("coderclaw-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createCoderClawCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
