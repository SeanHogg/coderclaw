...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): CoderClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-workspace-"));
    const now = Date.now();
