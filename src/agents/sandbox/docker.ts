...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "coderclaw.sandbox=1");
  args.push("--label", `coderclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `coderclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `coderclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "coderclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`coderclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`coderclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("coderclaw sandbox recreate --all");
}
