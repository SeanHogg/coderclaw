  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          coderclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.CODERCLAW_GATEWAY_PORT;
    process.env.CODERCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.CODERCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.CODERCLAW_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.CODERCLAW_GATEWAY_PORT = prevGatewayPort;
    }
    if (prevGatewayToken === undefined) {
      delete process.env.CODERCLAW_GATEWAY_TOKEN;
    } else {
      process.env.CODERCLAW_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.CODERCLAW_GATEWAY_PASSWORD;
    } else {
      process.env.CODERCLAW_GATEWAY_PASSWORD = prevGatewayPassword;
    }
