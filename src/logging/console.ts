...
import { getLogger, type LoggerSettings } from "./file.js";
...
type ConsoleConfigLoader = () => CoderClawConfig["logging"] | undefined;
const loadConfigFallbackDefault: ConsoleConfigLoader = () => {
  try {
    const loaded = requireConfig("../config/config.js") as {
      loadConfig?: () => CoderClawConfig;
    };
...
function normalizeConsoleLevel(level?: string): LogLevel {
  ...
  if (!level && process.env.VITEST === "true" && process.env.CODERCLAW_TEST_CONSOLE !== "1") {
    return "silent";
  }
...
function resolveConsoleSettings(): ConsoleSettings {
  let cfg: CoderClawConfig["logging"] | undefined =
