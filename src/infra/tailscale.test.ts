...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.CODERCLAW_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.CODERCLAW_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.CODERCLAW_TEST_TAILSCALE_BINARY;
    } else {
      process.env.CODERCLAW_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
