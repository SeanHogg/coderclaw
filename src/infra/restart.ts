...
export function triggerCoderClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.CODERCLAW_SYSTEMD_UNIT,
        process.env.CODERCLAW_PROFILE,
      );
  ...
  const label =
    process.env.CODERCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CODERCLAW_PROFILE);
