...
import { resolveCoderClawPackageRoot, resolveCoderClawPackageRootSync } from "./coderclaw-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveCoderClawPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "coderclaw" + dist/control-ui/index.html
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (fs.existsSync(pkgJsonPath)) {
      try {
        ...
        if (parsed.name === "coderclaw" || parsed.name === "coderclaw") {
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveCoderClawPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // coderclaw.mjs or dist/<bundle>.js
