...
import type { CoderClawConfig, ConfigFileSnapshot } from "../config/config.js";
...
async function readPluginManifestExtensions(pluginPath: string): Promise<string[]> {
  ...
  type AnyManifestKey = (typeof LEGACY_MANIFEST_KEYS)[number];
  const parsed = JSON.parse(raw) as Partial<
    Record<AnyManifestKey, { extensions?: unknown }>
  > | null;
  const extensions = (
    parsed?.[MANIFEST_KEY] ?? LEGACY_MANIFEST_KEYS.map((k) => parsed?.[k]).find(Boolean)
  )?.extensions;
...
export async function collectPluginsTrustFindings(params: {
  ...
  if (enabledExtensionPluginIds.length > 0) {
    ...
    for (const context of contexts) {
      ...
      const broadPolicy = isToolAllowedByPolicies("__coderclaw_plugin_probe__", policies);
  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `coderclaw security audit --deep`.",
      });
    },
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all coderclaw.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your CoderClaw extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (entry.skill.source === "coderclaw-bundled") {
        continue;
      }
      ...
      const summary = await skillScanner.scanDirectoryWithSummary(skillDir).catch((err) => {
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `coderclaw security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
