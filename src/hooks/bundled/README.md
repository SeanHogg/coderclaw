...
This directory contains hooks that ship with CoderClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.coderclaw/coderclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'coderclaw.*gateway' && pnpm coderclaw gateway`
3. Enable the hook: `coderclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.coderclaw.ai/automation/hooks
