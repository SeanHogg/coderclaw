...
homepage: https://docs.coderclaw.ai/automation/hooks#command-logger
metadata:
  {
    "coderclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.coderclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.coderclaw/logs/commands.log ~/.coderclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/coderclaw`:
   ```
   /home/username/.coderclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
