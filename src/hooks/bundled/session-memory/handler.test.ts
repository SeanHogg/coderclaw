...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => CoderClawConfig;
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("coderclaw-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): CoderClawConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
