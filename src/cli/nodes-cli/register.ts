    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["coderclaw nodes status", "List known nodes with live status."],
          ["coderclaw nodes pairing pending", "Show pending node pairing requests."],
          ['coderclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["coderclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.coderclaw.ai/cli/nodes")}\n`,
    );
