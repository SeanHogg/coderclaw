...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'coderclaw-gateway.service'");
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.coderclaw.gateway'");
      expect(content).toContain('rm -f "$0"');
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "CoderClaw Gateway"');
      expect(content).toContain('schtasks /Run /TN "CoderClaw Gateway"');
      // Batch self-cleanup
    ...
    it("uses CODERCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      const scriptPath = await prepareRestartScript({
        CODERCLAW_PROFILE: "default",
        CODERCLAW_WINDOWS_TASK_NAME: "CoderClaw Gateway (custom)",
      });

      expect(scriptPath).toBeTruthy();
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain('schtasks /End /TN "CoderClaw Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "CoderClaw Gateway (custom)"');
    ...
    it("uses custom profile in service names", async () => {
      ...
      expect(scriptPath).toBeTruthy();
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain("coderclaw-gateway-production.service");
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      expect(scriptPath).toBeTruthy();
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain("gui/502/ai.coderclaw.staging");
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(scriptPath).toBeTruthy();
      const content = await fs.readFile(scriptPath!, "utf-8");
      expect(content).toContain('schtasks /End /TN "CoderClaw Gateway (production)"');
