...
import type { ConfigFileSnapshot, CoderClawConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `coderclaw config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<(cfg: CoderClawConfig) => Promise<void>>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: CoderClawConfig) => mockWriteConfigFile(cfg),
}));
...
function setSnapshot(resolved: CoderClawConfig, config: CoderClawConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: CoderClawConfig = {
        agents: {
      ...
      const runtimeMerged: CoderClawConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
