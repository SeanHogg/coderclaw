...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.coderclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + CoderClaw hooks")
    ...
    .option("--hook-url <url>", "CoderClaw hook URL")
    .option("--hook-token <token>", "CoderClaw hook token")
