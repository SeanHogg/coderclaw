...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.CODERCLAW_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service CODERCLAW_GATEWAY_TOKEN does not match gateway.auth.token in coderclaw.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `coderclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
