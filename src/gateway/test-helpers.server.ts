...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.CODERCLAW_STATE_DIR;
  previousConfigPath = process.env.CODERCLAW_CONFIG_PATH;
  previousSkipBrowserControl = process.env.CODERCLAW_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.CODERCLAW_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.CODERCLAW_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.CODERCLAW_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.CODERCLAW_SKIP_CHANNELS;
  previousSkipProviders = process.env.CODERCLAW_SKIP_PROVIDERS;
  previousSkipCron = process.env.CODERCLAW_SKIP_CRON;
  previousMinimalGateway = process.env.CODERCLAW_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.CODERCLAW_STATE_DIR = path.join(tempHome, ".coderclaw");
  delete process.env.CODERCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.CODERCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.CODERCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.CODERCLAW_SKIP_CANVAS_HOST = "1";
  process.env.CODERCLAW_SKIP_CHANNELS = "1";
  process.env.CODERCLAW_SKIP_PROVIDERS = "1";
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.CODERCLAW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.CODERCLAW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.CODERCLAW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.CODERCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.CODERCLAW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.CODERCLAW_SKIP_CANVAS_HOST;
    } else {
      process.env.CODERCLAW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.CODERCLAW_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.CODERCLAW_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.CODERCLAW_SKIP_CHANNELS;
    } else {
      process.env.CODERCLAW_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.CODERCLAW_SKIP_PROVIDERS;
    } else {
      process.env.CODERCLAW_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.CODERCLAW_SKIP_CRON;
    } else {
      process.env.CODERCLAW_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.CODERCLAW_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.CODERCLAW_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["CODERCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.CODERCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.CODERCLAW_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.CODERCLAW_GATEWAY_PASSWORD;
