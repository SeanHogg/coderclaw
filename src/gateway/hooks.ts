...
export function resolveHooksConfig(cfg: CoderClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: CoderClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-coderclaw-token"] === "string"
      ? req.headers["x-coderclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
