...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.CODERCLAW_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "coderclaw");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.coderclaw-tool-probe.${nonceA}.txt`);
      ...
      try {
        ...
        process.env.CODERCLAW_CONFIG_PATH = prev.configPath;
        process.env.CODERCLAW_GATEWAY_TOKEN = prev.token;
        process.env.CODERCLAW_SKIP_CHANNELS = prev.skipChannels;
        process.env.CODERCLAW_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.CODERCLAW_SKIP_CRON = prev.skipCron;
        process.env.CODERCLAW_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.CODERCLAW_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.CODERCLAW_STATE_DIR = prev.stateDir;
      process.env.CODERCLAW_CONFIG_PATH = prev.configPath;
      process.env.CODERCLAW_GATEWAY_TOKEN = prev.token;
      process.env.CODERCLAW_SKIP_CHANNELS = prev.skipChannels;
      process.env.CODERCLAW_SKIP_GMAIL_WATCHER = prev.skipGmail;
