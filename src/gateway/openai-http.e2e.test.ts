    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "coderclaw", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-coderclaw-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-coderclaw-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "coderclaw", messages: [{ role: "user", content: "hi" }] },
          {
            "x-coderclaw-agent-id": "beta",
            "x-coderclaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
