...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveCoderClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("coderclaw models auth setup-token")}\` or \`${formatCliCommand("coderclaw configure")}\`.`
          : `Run \`${formatCliCommand("coderclaw configure")}\` or set an API key env var.`;
