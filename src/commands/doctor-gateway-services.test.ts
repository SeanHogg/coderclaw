...
function setupGatewayTokenRepairScenario(expectedToken: string) {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      CODERCLAW_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service CODERCLAW_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
  mocks.buildGatewayInstallPlan.mockResolvedValue({
    ...
    environment: {
      CODERCLAW_GATEWAY_TOKEN: expectedToken,
    },
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("uses CODERCLAW_GATEWAY_TOKEN when config token is missing", async () => {
    const previousToken = process.env.CODERCLAW_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousToken === undefined) {
        ...
        process.env.CODERCLAW_GATEWAY_TOKEN = previousToken;
      }
    }
  });
});
