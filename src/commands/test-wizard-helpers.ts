...
export function requireCoderClawAgentDir(): string {
  const agentDir = process.env.CODERCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("CODERCLAW_AGENT_DIR not set");
  }
  return agentDir;
}
