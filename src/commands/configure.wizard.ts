...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.CODERCLAW_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.CODERCLAW_GATEWAY_PASSWORD;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from coderclaw.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "CoderClaw update wizard" : "CoderClaw configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: CoderClawConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("coderclaw doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const newPassword =
      nextConfig.gateway?.auth?.password ?? process.env.CODERCLAW_GATEWAY_PASSWORD;
    const oldPassword =
      baseConfig.gateway?.auth?.password ?? process.env.CODERCLAW_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.CODERCLAW_GATEWAY_TOKEN;
