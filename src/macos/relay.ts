...
declare const __CODERCLAW_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __CODERCLAW_VERSION__ === "string" && __CODERCLAW_VERSION__) ||
  process.env.CODERCLAW_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  ...
  const { ensureCoderClawCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[coderclaw] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
