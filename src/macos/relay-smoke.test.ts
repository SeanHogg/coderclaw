...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { CODERCLAW_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { CODERCLAW_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports CODERCLAW_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { CODERCLAW_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { CODERCLAW_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { CODERCLAW_SMOKE: "nope" })).toBe("qr");
  });
});
