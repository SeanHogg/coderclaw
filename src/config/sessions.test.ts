...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "coderclaw-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-coderclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-coderclaw#general");
  });
  ...
  it("derives session transcripts dir from CODERCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CODERCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.CODERCLAW_STATE_DIR = "/custom/state";
    try {
  ...
  it("resolves cross-agent paths when CODERCLAW_STATE_DIR differs from stored paths", () => {
    const prev = process.env.CODERCLAW_STATE_DIR;
    process.env.CODERCLAW_STATE_DIR = path.resolve("/different/state");
    try {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    const prev = process.env.CODERCLAW_STATE_DIR;
    process.env.CODERCLAW_STATE_DIR = path.resolve("/home/user/.coderclaw");
    try {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
