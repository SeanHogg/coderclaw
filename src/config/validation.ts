...
import type { CoderClawConfig, ConfigValidationIssue } from "./types.js";
import { CoderClawSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: CoderClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: CoderClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = CoderClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as CoderClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as CoderClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as CoderClawConfig,
  };
}
