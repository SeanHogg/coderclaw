# CoderClaw ACP Bridge

This document describes how the CoderClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`coderclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
CoderClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `coderclaw acp` and speaks ACP messages over stdio.
...
`coderclaw acp` resolves the Gateway URL and auth from CLI flags or config:
