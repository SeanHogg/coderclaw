#!/bin/bash
# Setup CoderClaw Auth Management System
...
echo "=== CoderClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'coderclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  CoderClaw message: Send warning via CoderClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/coderclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/coderclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/coderclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now coderclaw-auth-monitor.timer
...
echo "   ssh phone 'chmod +x ~/.shortcuts/CoderClaw*'"
echo ""
...
echo "  - Auth monitor: systemctl --user status coderclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
