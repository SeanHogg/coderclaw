...
const shouldBuild = (deps) => {
  if (deps.env.CODERCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.CODERCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[coderclaw] ${message}\n`);
};

const runCoderClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["coderclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runCoderClaw(deps);
  }
