...
# Then:
#   ./scripts/run-coderclaw-podman.sh launch           # Start gateway
...
LAUNCH_SCRIPT="$CODERCLAW_HOME/run-coderclaw-podman.sh"
...
if [[ -z "${CODERCLAW_GATEWAY_TOKEN:-}" ]]; then
  export CODERCLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "CODERCLAW_GATEWAY_TOKEN" "$CODERCLAW_GATEWAY_TOKEN"
  echo "Generated CODERCLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

# The gateway refuses to start unless gateway.mode=local is set in config.
# Keep this minimal; users can run the wizard later to configure channels/providers.
CONFIG_JSON="$CONFIG_DIR/coderclaw.json"
...
PODMAN_USERNS="${CODERCLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported CODERCLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (CODERCLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi
