...
BASE_IMAGE="${BASE_IMAGE:-coderclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-coderclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=coderclaw.sandbox=1)
NOTE
