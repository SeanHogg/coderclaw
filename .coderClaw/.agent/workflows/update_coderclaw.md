---
description: Update CoderClaw from upstream when branch has diverged (ahead/behind)
---

# CoderClaw Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "CoderClaw" || true

# Move old version
mv /Applications/CoderClaw.app /tmp/CoderClaw-backup.app

# Install new build
cp -R dist/CoderClaw.app /Applications/

# Launch
open /Applications/CoderClaw.app
```
...
pnpm coderclaw health

# Verify no zombie processes
ps aux | grep -E "(coderclaw|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm coderclaw agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update coderclaw.json with fallback chains
...
pnpm coderclaw agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
