...
function parseMemoryFile(filePath, fileName) {
  ...
  return entries.map(entry => {
    ...
    const wordTags = cont.match(/\b(protocol|suggestion|memory|conflict|error|session|cron|heartbeat|decision|rejected|approved|roadmap|lessons?)\b/gi)
      ? cont.match(/\b(protocol|suggestion|memory|conflict|error|session|cron|heartbeat|decision|rejected|approved|roadmap|lessons?)\b/gi).map(t => t.toLowerCase())
      : [];
    const tags = [...new Set([...hashtagTags, ...wordTags])];

    const wordCount = clean.split(/\s+/).filter(w => w.length > 0).length;

    return { ...entry, summary, tags, wordCount, clusterId: null, lastAccessed: new Date().toISOString() };
  });
}
