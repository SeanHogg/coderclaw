...
Assess and harden the host running CoderClaw, then align it to a user-defined risk tolerance without breaking access. Use CoderClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim CoderClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. CoderClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if CoderClaw can access sensitive data.
...
### 2) Run CoderClaw security audits (read-only)

As part of the default read-only checks, run `coderclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `coderclaw security audit` (faster, non-probing)
2. `coderclaw security audit --json` (structured output)

Offer to apply CoderClaw safe defaults (numbered):

1. `coderclaw security audit --fix`

Be explicit that `--fix` only tightens CoderClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check CoderClaw version/update status (read-only)

As part of the default read-only checks, run `coderclaw update status`.
...
- Credential hygiene notes (location of CoderClaw creds, prefer disk encryption)
...
- CoderClaw security audit (re-run)
...
After CoderClaw install or first hardening pass, run at least one baseline audit and version check:

- `coderclaw security audit`
- `coderclaw security audit --deep`
- `coderclaw update status`

Ongoing monitoring is recommended. Use the CoderClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `coderclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `coderclaw update status`
...
Before creating, `coderclaw cron list` and match on exact `name`. If found, `coderclaw cron edit <id> ...`.
If not found, `coderclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `coderclaw update status` (preferred for source checkouts and channels)
2. `npm view coderclaw version` (published npm version)

## CoderClaw command accuracy

Use only supported commands and flags:

- `coderclaw security audit [--deep] [--fix] [--json]`
- `coderclaw status` / `coderclaw status --deep`
- `coderclaw health --json`
- `coderclaw update status`
- `coderclaw cron add|list|runs|run`

Do not invent CLI flags or imply CoderClaw enforces host firewall/SSH policies.
